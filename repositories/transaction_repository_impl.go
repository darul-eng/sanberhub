...
func (repository *TransactionRepositoryImpl) Find(ctx context.Context, tx *sql.Tx, savingAccountID int) ([]domain.Transaction, error) {
	SQL := `SELECT "user_id", "saving_account_id", "transaction_code", "amount", "created_at" FROM "transaction" WHERE "saving_account_id" = $1`
	...
	for rows.Next() {
		...
		err := rows.Scan(&transaction.UserID, &transaction.SavingAccountID, &transaction.TransactionCode, &transaction.Amount, &transaction.CreatedAt)
		helpers.PanicIfError(err)
